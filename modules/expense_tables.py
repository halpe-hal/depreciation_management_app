...
from db.all_expense_depreciation import get_expenses, add_expense, delete_expense, update_expense_totals_by_category
                    ...
                    if row["操作"] == "削除" and not pd.isna(row.get("id")):
                        ...
                        if not old.empty:
                            ...
                            if any([
                                ...
                                supabase.table("all_expense_depreciation").update({
                                    "partner": row["取引先"],
                                    "account": row["勘定項目"],
                                    "detail": row["詳細"],
                                    "payment": row["支払方法"],
...
# def show_all_expense_depreciation_tables(year: int, month: int, top_category: str):
