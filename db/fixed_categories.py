...
def apply_fixed_expenses(year: int, month: int, selected_top_category: str) -> bool:
    ...
    all_expense_depreciation テーブルに、second_category（家賃・その他固定費・融資返済）として保存する。
    """
    from db.all_expense_depreciation import get_expenses  # 循環import防止
    ...
    try:
        ...
        if new_rows:
            supabase.table("all_expense_depreciation").insert(new_rows).execute()
