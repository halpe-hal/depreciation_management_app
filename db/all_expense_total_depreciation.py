...
def save_expense_totals(year: int, month: int, top_category: str, totals: dict) -> bool:
    """カテゴリごとの出金合計をall_expense_total_depreciationテーブルに保存（上書き）"""
    try:
        ...
        supabase.table("all_expense_total_depreciation").insert(payload).execute()
        return True
...
def get_expense_totals(year: int, month: int, top_category: str) -> dict:
    ...
    try:
        res = supabase.table("all_expense_total_depreciation")\
            .select("*")\
            .eq("year", year)\
            .eq("month", month)\
            .eq("top_category", top_category)\
...
def get_expense_totals_batch(years: list, top_category: str) -> list:
    ...
    """
    try:
        res = supabase.table("all_expense_total_depreciation").select("*")\
...
def get_expense_totals_all(years: list) -> list:
    ...
    try:
        ...
        while True:
            query = supabase.table("all_expense_total_depreciation")\
                .select("*")\
                .in_("year", years)\
                .order("id")\
